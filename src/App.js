  ...
  return (
    ...
    <><nav className="navbar">
      <div className="navbar-container">
        <h1 className="navbar-title">Application Survey</h1>
        {isAuthenticated && (
          <div className="navbar-user-container">
            <p className="navbar-user">User: {user.name}</p>
            <button className="navbar-logout" onClick={handleLogout}>Log Out</button>
          </div>
        )}
      </div>
    </nav><div className="survey-container">
        {isAuthenticated ? (
          <div>
                  ...
                  {components.length > 0 && (
                    <form onSubmit={handleSubmit}>
                      {questions.map((question) => (
                        <div
                          key={question.name}
                          className={showDescriptions[question.name]
